...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex sticky top-0 h-screen w-20 md:w-24 bg-[#EAA208] flex-col items-center py-6 px-2 shrink-0 shadow-lg select-none text-slate-900 z-30 transition-colors">
      ...
      <nav 
        ...
        className="md:hidden fixed bottom-0 inset-x-0 z-40 bg-[#EAA208] border-t border-amber-600/30 shadow-2xl px-2 py-1 flex items-center justify-around select-none pb-[max(env(safe-area-inset-bottom),0.375rem)]"
      >
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive = activeTab === item.id;
            ...
            <button
              ...
              className={`relative flex flex-col items-center justify-center flex-1 py-1.5 px-1 rounded-xl transition-all ${
                isActive
                  ...
                  : 'text-slate-950/80 active:bg-amber-600/20'
              }`}
              aria-label={item.label}
            >
              <div className="relative">
                ...
                {item.badge !== undefined && (
                  <span className="absolute -top-1.5 -right-2 bg-slate-950 text-amber-400 text-[9px] font-mono font-bold min-w-4 h-4 px-1 rounded-full flex items-center justify-center border border-[#EAA208]">
                    {item.badge}
                  </span>
                )}
              </div>
              <span className="text-[10px] font-bold mt-0.5 truncate max-w-[62px] leading-tight">
                {item.id === 'explore' ? 'สำรวจ' :
                 item.id === 'lineage' ? 'สายต่อยอด' :
                 item.id === 'analytics' ? 'สถิติ' :
                 item.id === 'favorites' ? 'บันทึกไว้' : 'ของฉัน'}
              </span>
            </button>
          );
        })}
